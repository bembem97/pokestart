...
const buttonVariants = cva(
  [
    "group/button inline-flex shrink-0 items-center justify-center rounded-none border border-transparent bg-clip-padding text-xs font-semibold tracking-widest whitespace-nowrap uppercase transition-all duration-75 outline-none select-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/30 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-2 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
    "not-dark:hover:scale-102 not-dark:hover:shadow not-dark:hover:shadow-foreground/50",
  ],
  {
    variants: {
      variant: {
        ...
        default: [
          ...
          "dark:hover:border-primary/75 dark:hover:text-primary",
        ],
        outline:
        ...
        ghost:
          "hover:bg-muted hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground dark:hover:bg-muted/25",
        destructive:
...
function Button({
  ...
  return (
    <ButtonPrimitive
      data-slot="button"
      className={cn(buttonVariants({ corner, variant, size, className }))}
      {...props}
    />
  )
}
